// 表示言語（英語／やさしい日本語／ベトナム語／ネパール語）。漢字そのものの表示とは別
// 注: ベトナム語・ネパール語の訳は母語話者の確認を受けていない
import { createContext, useContext } from 'react'

export type Language = 'en' | 'ja' | 'vi' | 'ne'

/** 言語の選択肢に出す名前（それぞれの言語で書く） */
export const LANGUAGE_NAMES: Record<Language, string> = {
  en: 'English',
  ja: 'やさしい にほんご',
  vi: 'Tiếng Việt',
  ne: 'नेपाली',
}

const en = {
  navHome: 'Home',
  appTitle: 'Kanji Shape Trainer',
  appTagline: 'Learn to see each kanji as one shape. No reading or meaning needed — just look and find.',
  startPractice: 'Start practice',
  stagesTitle: 'Practice modes',
  descLv1: 'Find the kanji that matches the sample.',
  descLv2: 'Find every copy of the sample on the board.',
  descLv3: 'See the sample for a moment, then choose it.',
  descLv4: 'Find the one kanji that is different.',
  descLv5: 'Put two parts together and choose the kanji.',
  languageLabel: 'Language',
  navPractice: 'Practice',
  navRecords: 'Records',
  navCredits: 'Credits',
  instruction: 'Find the same kanji.',
  stageFindOne: 'Find one',
  stageFindAll: 'Find all',
  stageQuickLook: 'Quick look',
  stageOddOneOut: 'Odd one out',
  stageBuild: 'Build it',
  buildEmpty: 'There are no kanji to build yet. Practice a little more in the other modes and more kanji will appear here.',
  startTitle: 'Where to start',
  startBeginner: 'New to kanji',
  startSome: 'I know some',
  startWell: 'I know many',
  startHint: 'You can change this at any time. Your records are kept.',
  confirmStart: 'Start again from this point? Your records are kept.',
  instructionBuild: 'Put the two parts together. Which kanji do they make?',
  instructionOdd: 'One kanji is different from the others. Tap it.',
  legendOdd: 'The different one',
  instructionFlash: 'Remember the kanji. It disappears soon, then choose the same one.',
  showTime: 'Showing time',
  instructionAll: 'Tap every kanji that is the same as the sample, then press Done.',
  done: 'Done',
  legendFound: 'Found',
  legendMissed: 'Missed',
  legendWrong: 'Wrong choice',
  sampleCorrect: 'Sample = correct',
  yourChoice: 'Your choice',
  next: 'Next',
  sessionComplete: 'Session complete',
  averageTime: 'Average time',
  seconds: 's',
  continue: 'Continue',
  recordsTitle: 'Your records',
  learningCount: 'Kanji you are practicing',
  noSessions: 'No sessions yet. Finish a 10-question session to see your progress.',
  chartTime: 'Average time per question (seconds)',
  chartCorrect: 'Correct answers (%)',
  session: 'Session',
  correctCount: 'Correct',
  deleteRecords: 'Delete records',
  confirmDelete: 'Delete all your records and start over?',
  introTitle: 'Welcome',
  introBody:
    'Look at the kanji at the top and tap the same one below. You do not need to know how to read it. Your records are saved only on this device. They are not shared with other devices.',
  introOk: 'Start',
  creditsTitle: 'Credits',
  creditsIntro: 'This app uses the following data and fonts. Thank you to their authors.',
  noticesLink: 'Full license texts',
}

export type Messages = typeof en

const ja: Messages = {
  navHome: 'ホーム',
  appTitle: 'かんじの かたち れんしゅう',
  appTagline: 'かんじを 1つの かたちとして みる れんしゅうです。よみかたや いみは いりません。みて、さがす だけです。',
  startPractice: 'れんしゅうを はじめる',
  stagesTitle: 'れんしゅうの しゅるい',
  descLv1: 'みほんと おなじ かんじを 1つ えらびます。',
  descLv2: 'みほんと おなじ かんじを ぜんぶ えらびます。',
  descLv3: 'みほんを すこしだけ みて、おなじ かんじを えらびます。',
  descLv4: '1つだけ ちがう かんじを さがします。',
  descLv5: '2つの ぶひんを あわせた かんじを えらびます。',
  languageLabel: 'ことば',
  navPractice: 'れんしゅう',
  navRecords: 'きろく',
  navCredits: 'しゅってん',
  instruction: 'おなじ かんじを えらんで ください。',
  stageFindOne: '1つ さがす',
  stageFindAll: 'ぜんぶ さがす',
  stageQuickLook: 'いっしゅん みる',
  stageOddOneOut: 'ちがう じを さがす',
  stageBuild: 'くみたてる',
  buildEmpty: 'まだ くみたてる かんじが ありません。ほかの れんしゅうを もう すこし すると、ここに でて きます。',
  startTitle: 'どこから はじめますか',
  startBeginner: 'はじめて',
  startSome: 'すこし しっている',
  startWell: 'よく しっている',
  startHint: 'いつでも かえられます。きろくは のこります。',
  confirmStart: 'ここから はじめなおしますか。きろくは のこります。',
  instructionBuild: '2つの ぶひんを あわせると、どの かんじに なりますか。',
  instructionOdd: '1つだけ ちがう かんじが あります。それを おして ください。',
  legendOdd: 'なかまはずれ',
  instructionFlash: 'かんじを おぼえて ください。すぐに きえます。そのあと おなじ かんじを えらんで ください。',
  showTime: 'みせる じかん',
  instructionAll: 'みほんと おなじ かんじを ぜんぶ えらんで、「できた」を おして ください。',
  done: 'できた',
  legendFound: 'みつけた',
  legendMissed: 'みおとし',
  legendWrong: 'ちがう じ',
  sampleCorrect: 'みほん（せいかい）',
  yourChoice: 'あなたが えらんだ じ',
  next: 'つぎへ',
  sessionComplete: 'おわりました',
  averageTime: 'へいきんの じかん',
  seconds: 'びょう',
  continue: 'つづける',
  recordsTitle: 'あなたの きろく',
  learningCount: 'いま れんしゅうしている かんじの かず',
  noSessions: 'まだ きろくが ありません。10もん とくと、ここに でます。',
  chartTime: '1もんの へいきんの じかん（びょう）',
  chartCorrect: 'せいかいの わりあい（%）',
  session: 'かい',
  correctCount: 'せいかい',
  deleteRecords: 'きろくを けす',
  confirmDelete: 'きろくを ぜんぶ けして、さいしょから はじめますか。',
  introTitle: 'ようこそ',
  introBody:
    'うえの かんじと おなじ かんじを、したから えらんで ください。よみかたは しらなくて だいじょうぶです。きろくは この きかいの なかにだけ のこります。ほかの きかいには ひきつがれません。',
  introOk: 'はじめる',
  creditsTitle: 'しゅってん',
  creditsIntro: 'この アプリは つぎの データと フォントを つかっています。',
  noticesLink: 'ライセンスの ぜんぶの ぶん',
}

const vi: Messages = {
  navHome: 'Trang chủ',
  appTitle: 'Luyện nhận diện chữ Hán',
  appTagline: 'Luyện nhìn mỗi chữ Hán như một hình khối. Không cần biết cách đọc hay nghĩa — chỉ cần nhìn và tìm.',
  startPractice: 'Bắt đầu luyện tập',
  stagesTitle: 'Các kiểu luyện tập',
  descLv1: 'Chọn chữ giống với chữ mẫu.',
  descLv2: 'Tìm tất cả các chữ giống chữ mẫu trên bảng.',
  descLv3: 'Nhìn chữ mẫu trong giây lát, rồi chọn chữ đó.',
  descLv4: 'Tìm một chữ khác với các chữ còn lại.',
  descLv5: 'Ghép hai bộ phận lại và chọn chữ đúng.',
  languageLabel: 'Ngôn ngữ',
  navPractice: 'Luyện tập',
  navRecords: 'Kết quả',
  navCredits: 'Nguồn',
  instruction: 'Hãy chọn chữ giống chữ mẫu.',
  stageFindOne: 'Tìm một chữ',
  stageFindAll: 'Tìm tất cả',
  stageQuickLook: 'Nhìn nhanh',
  stageOddOneOut: 'Tìm chữ khác',
  stageBuild: 'Ghép chữ',
  buildEmpty: 'Chưa có chữ nào để ghép. Hãy luyện thêm ở các kiểu khác, rồi chữ sẽ xuất hiện ở đây.',
  startTitle: 'Bắt đầu từ đâu',
  startBeginner: 'Mới học chữ Hán',
  startSome: 'Biết một ít',
  startWell: 'Biết nhiều',
  startHint: 'Bạn có thể đổi bất cứ lúc nào. Kết quả vẫn được giữ lại.',
  confirmStart: 'Bắt đầu lại từ vị trí này? Kết quả vẫn được giữ lại.',
  instructionBuild: 'Ghép hai bộ phận lại. Chúng tạo thành chữ nào?',
  instructionOdd: 'Có một chữ khác với các chữ còn lại. Hãy chạm vào chữ đó.',
  legendOdd: 'Chữ khác',
  instructionFlash: 'Hãy nhớ chữ này. Chữ sẽ biến mất ngay, sau đó hãy chọn chữ giống vậy.',
  showTime: 'Thời gian hiển thị',
  instructionAll: 'Chạm vào tất cả các chữ giống chữ mẫu, rồi nhấn Xong.',
  done: 'Xong',
  legendFound: 'Đã tìm thấy',
  legendMissed: 'Bỏ sót',
  legendWrong: 'Chọn sai',
  sampleCorrect: 'Chữ mẫu = đáp án',
  yourChoice: 'Bạn đã chọn',
  next: 'Tiếp',
  sessionComplete: 'Hoàn thành',
  averageTime: 'Thời gian trung bình',
  seconds: 'giây',
  continue: 'Tiếp tục',
  recordsTitle: 'Kết quả của bạn',
  learningCount: 'Số chữ đang luyện',
  noSessions: 'Chưa có kết quả. Hãy làm xong một lượt 10 câu để xem tiến bộ.',
  chartTime: 'Thời gian trung bình mỗi câu (giây)',
  chartCorrect: 'Tỉ lệ đúng (%)',
  session: 'Lượt',
  correctCount: 'Số câu đúng',
  deleteRecords: 'Xóa kết quả',
  confirmDelete: 'Xóa toàn bộ kết quả và bắt đầu lại?',
  introTitle: 'Chào mừng',
  introBody:
    'Nhìn chữ Hán ở trên và chọn chữ giống vậy ở dưới. Bạn không cần biết cách đọc. Kết quả chỉ được lưu trên thiết bị này, không chuyển sang thiết bị khác.',
  introOk: 'Bắt đầu',
  creditsTitle: 'Nguồn',
  creditsIntro: 'Ứng dụng này sử dụng các dữ liệu và phông chữ sau. Xin cảm ơn các tác giả.',
  noticesLink: 'Toàn văn giấy phép',
}

const ne: Messages = {
  navHome: 'गृहपृष्ठ',
  appTitle: 'कान्जी आकार अभ्यास',
  appTagline: 'हरेक कान्जीलाई एउटै आकारको रूपमा हेर्ने अभ्यास। पढाइ वा अर्थ जान्नु पर्दैन — हेर्नुहोस् र खोज्नुहोस्।',
  startPractice: 'अभ्यास सुरु गर्नुहोस्',
  stagesTitle: 'अभ्यासका प्रकार',
  descLv1: 'नमूनासँग मिल्ने कान्जी छान्नुहोस्।',
  descLv2: 'बोर्डमा नमूनासँग मिल्ने सबै कान्जी खोज्नुहोस्।',
  descLv3: 'नमूना छिनभर हेर्नुहोस्, अनि त्यही कान्जी छान्नुहोस्।',
  descLv4: 'अरूभन्दा फरक एउटा कान्जी खोज्नुहोस्।',
  descLv5: 'दुई भाग जोडेर बन्ने कान्जी छान्नुहोस्।',
  languageLabel: 'भाषा',
  navPractice: 'अभ्यास',
  navRecords: 'नतिजा',
  navCredits: 'स्रोत',
  instruction: 'नमूनासँग मिल्ने कान्जी छान्नुहोस्।',
  stageFindOne: 'एउटा खोज्नुहोस्',
  stageFindAll: 'सबै खोज्नुहोस्',
  stageQuickLook: 'छिटो हेर्नुहोस्',
  stageOddOneOut: 'फरक खोज्नुहोस्',
  stageBuild: 'जोड्नुहोस्',
  buildEmpty: 'अहिले जोड्ने कान्जी छैन। अरू अभ्यास अलि बढी गर्नुहोस्, अनि यहाँ कान्जी देखिनेछन्।',
  startTitle: 'कहाँबाट सुरु गर्ने',
  startBeginner: 'कान्जी नयाँ हो',
  startSome: 'अलिअलि जान्दछु',
  startWell: 'धेरै जान्दछु',
  startHint: 'जुनसुकै बेला परिवर्तन गर्न सकिन्छ। नतिजा राखिन्छ।',
  confirmStart: 'यहाँबाट फेरि सुरु गर्ने? नतिजा राखिन्छ।',
  instructionBuild: 'दुई भाग जोड्नुहोस्। कुन कान्जी बन्छ?',
  instructionOdd: 'एउटा कान्जी अरूभन्दा फरक छ। त्यसलाई थिच्नुहोस्।',
  legendOdd: 'फरक कान्जी',
  instructionFlash: 'कान्जी सम्झनुहोस्। यो छिट्टै हराउँछ, अनि त्यही कान्जी छान्नुहोस्।',
  showTime: 'देखाउने समय',
  instructionAll: 'नमूनासँग मिल्ने सबै कान्जी थिच्नुहोस्, अनि "सकियो" थिच्नुहोस्।',
  done: 'सकियो',
  legendFound: 'भेटियो',
  legendMissed: 'छुट्यो',
  legendWrong: 'गलत छनोट',
  sampleCorrect: 'नमूना = सही उत्तर',
  yourChoice: 'तपाईंको छनोट',
  next: 'अर्को',
  sessionComplete: 'सकियो',
  averageTime: 'औसत समय',
  seconds: 'सेकेन्ड',
  continue: 'जारी राख्नुहोस्',
  recordsTitle: 'तपाईंको नतिजा',
  learningCount: 'अभ्यास गरिरहेका कान्जी',
  noSessions: 'अहिलेसम्म नतिजा छैन। प्रगति हेर्न १० प्रश्नको एउटा चरण पूरा गर्नुहोस्।',
  chartTime: 'प्रति प्रश्न औसत समय (सेकेन्ड)',
  chartCorrect: 'सही उत्तर (%)',
  session: 'चरण',
  correctCount: 'सही',
  deleteRecords: 'नतिजा मेटाउनुहोस्',
  confirmDelete: 'सबै नतिजा मेटाएर फेरि सुरु गर्ने?',
  introTitle: 'स्वागत छ',
  introBody:
    'माथिको कान्जी हेर्नुहोस् र तल त्यस्तै कान्जी थिच्नुहोस्। पढ्न जान्नु पर्दैन। नतिजा यही उपकरणमा मात्र राखिन्छ, अरू उपकरणमा सर्दैन।',
  introOk: 'सुरु गर्नुहोस्',
  creditsTitle: 'स्रोत',
  creditsIntro: 'यो एपले तलका डेटा र फन्ट प्रयोग गर्छ। लेखकहरूलाई धन्यवाद।',
  noticesLink: 'लाइसेन्सको पूरा पाठ',
}

export const messages: Record<Language, Messages> = { en, ja, vi, ne }

export const MessagesContext = createContext<Messages>(en)
export const useMessages = () => useContext(MessagesContext)
